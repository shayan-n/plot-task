...
export default function CollapseBox({ title, children, className='', defaultCollapse=false }) {
    ...
    return (
        <div className="w-80 select-none flex flex-col rounded-xl sketch-shadow overflow-hidden">
            <div 
                role="button"
                tabIndex={-1}
                onKeyDown={() => {}}
            ...
            <Collapse collapse={collapse} className={`px-5 py-3 ${className}`}>{children}</Collapse>
        </div>
    );
}